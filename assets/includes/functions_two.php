...
//Custom Functions BUY SELL BUTTONS
function Wo_GetSignalButton($fincode = 0, $signal = 'buy') {
    global $wo;

    if (empty($fincode) || !is_numeric($fincode) or $fincode < 0) {
        return false;
    }
    ...
    if ($signal === 'buy') {
        return Wo_LoadPage($buy_button);
    } else {
        return Wo_LoadPage($sell_button);
    }
}
