...
// SA EDIT
//Original : added 'comp_id' => 0,
function Wo_GetPosts($data = array('filter_by' => 'all', 'comp_id' => 0, 'after_post_id' => 0, 'page_id' => 0, 'group_id' => 0, 'publisher_id' => 0, 'limit' => 5, 'event_id' => 0, 'ad-id' => 0)) {
    global $wo, $sqlConnect;
    ...
    // Added new if function for stock quotes
    if (!empty($data['comp_id']) && is_numeric($data['comp_id']) && $data['comp_id'] > 0) {
        $data['comp_id']   = Wo_Secure($data['comp_id']);
        $Wo_company_publisher = Wo_CompanyCaching($data['comp_id']);
    }
    // End of new SA insert
    ...
    if (isset($Wo_publisher['user_id'])) {
        ...
        switch ($data['filter_by']) {
            ...
            case 'maps':
                ...
                break;
            // Start Added custom, but i think its not required. delete after discussion. Gives Poll Filter on timeline?
            case 'polls':
                $query_text .= " AND `poll_id` <> 0";
                break;
            // End of custom Insert
        }

        // Start Edited, but i think its not required. delete after discussion (Orginal below)
        ...
        //SA EDIT START Privacy made common for logged in user if he is publisher?? Revert to original later if required. original below ->
        ...
        // SA START INSERT FOR COMPANY QUOTES
        } else if (isset($Wo_company_publisher['fincode'])) {
        $page_id = Wo_Secure($Wo_company_publisher['fincode']);
        $query_text .= " AND (`fincode` = {$fincode}) AND `id` NOT IN (SELECT `fincode` from " . T_PINNED_POSTS . " WHERE `fincode` = {$fincode})";
        ...
        // SA EDIT START ORGINAL BELOW. AGAIN LOGGEN IN V/s NON LOGGED IN PRIVACY. Revert to original later
        ...
        // SA EDIT START. WHY THIS EDIT? Privacy funtion gives option for user to have post privacy, my friends, anonymous, everyone etc... Revert to original later
    ...
    //// IN DEVELOPERS FILE THE BELOW FUNCTION HAS BEEN DELETED, I'VE LET IT REMAIN.  THIS I THINK GIVES ABILITY TO POST ANONYMOUSLY. DISCUSS
    ...
    // IN DEVELOPERS FILE THE BELOW FUNCTION HAS BEEN DELETED, THIS GIVES ABILITY TO ADMIN TO APPROVE BLOGS BEFIRE PUBLISHING I'VE LET IT REMAIN. DISCUSS
    else{
