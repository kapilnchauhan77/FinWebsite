...
function Wo_CompanyCaching($fincode){
    ...
    if ($wo['config']['cacheSystem'] == 1) {
        $fetched_data = $cache->read($hashed_fincode . '_COMPANY_Data.tmp');
        if (empty($fetched_data)) {
            $sql          = mysqli_query($sqlConnect, $query_text);
            $fetched_data = mysqli_fetch_assoc($sql);
            $cache->write($hashed_fincode . '_COMPANY_Data.tmp', $fetched_data);
        }
    } else {
        $sql          = mysqli_query($sqlConnect, $query_text);
        $fetched_data = mysqli_fetch_assoc($sql);
    ...
    $fetched_data['url'] = Wo_SeoLink('index.php?link1=timeline&u=' . str_replace(" ", "_", $fetched_data['s_name']));
...
function get_extra_graph_data($fincode = 0, $extra_g_d, $fformat){
    ...
    if (empty($data)) {
        ...
        $query_one  = mysqli_query($sqlConnect, $query_text);
        $dates_ = mysqli_fetch_assoc($query_one);

        foreach ($extra_g_d as $column_name) {
            $query_text = "SELECT * FROM " . $column_name . " WHERE fincode = " . $fincode . " LIMIT 1";
            $query_one  = mysqli_query($sqlConnect, $query_text);
            $data[$column_name] = mysqli_fetch_assoc($query_one);
            // $data[] = $query_text;s
        }
...
function get_financial_data($fincode = 0, $tab_data = 'qtrly', $fformat = 'BNK', $type_to_get = 'standalone_'){
    global $sqlConnect, $cache;

    $data           = array();
    $fincode        = Wo_Secure($fincode);
    $hashed_fincode = md5($fincode);
    $final_data = $cache->read($hashed_fincode . $tab_data . $type_to_get . '_Finance_Data.tmp');
    if (empty($data)) {
        if($tab_data == 'qtrly' || $tab_data == 'annual'){
            if($fformat=="BNK"){
                $query_text = "SELECT `column_name`, `description` FROM " . T_QABNKP;
            } else{
                $query_text = "SELECT `column_name`, `description` FROM " . T_QAMANP;
            }
        }
        else{
            if($fformat=="BNK"){
                $query_text = "SELECT `column_name`, `description` FROM " . $tab_data . '_' . T_QABNKP;
            } else{
                $query_text = "SELECT `column_name`, `description` FROM " . $tab_data . '_' .  T_QAMANP;
            }
        }
        $query_one  = mysqli_query($sqlConnect, $query_text);
        while ($fetched_data = mysqli_fetch_assoc($query_one)) {
                $data[] = get_full_financial_data($fetched_data, $tab_data, $type_to_get, $fincode);
        };
        if (empty($data)) {
            return array();
        }

        if ($tab_data == "profit_loss"){
            $query_text = "SELECT `y1`, `y2`, `y3`, `y4`, `y5`, `y6`, `y7`, `y8`, `y9`, `y10`, `y11`, `y12` FROM  finance_dates WHERE fincode = " . $fincode . " LIMIT 1;";
        }
        else{
            $query_text = "SELECT `y1`, `y2`, `y3`, `y4`, `y5`, `y6`, `y7`, `y8`, `y9`, `y10`, `y11`, `y12` FROM " . $tab_data . "_dates WHERE fincode = " . $fincode . " LIMIT 1;";
        }
        // $dates_ = "SELECT `y1`, `y2`, `y3`, `y4`, `y5`, `y6`, `y7`, `y8`, `y9`, `y10`, `y11`, `y12` FROM " . $tab_data . "_dates WHERE fincode = " . $fincode . " LIMIT 1;";
        $query_one  = mysqli_query($sqlConnect, $query_text);
        $dates_ = mysqli_fetch_assoc($query_one);
...
function get_full_financial_data($fetched_data, $tab_data, $type_to_get, $fincode){
    global $sqlConnect;

    if($tab_data == 'qtrly' || $tab_data == 'annual'){
        $table_name = $tab_data . '_' . $type_to_get . $fetched_data["column_name"];
    } else{
        $table_name = $type_to_get . $fetched_data["column_name"];
    }
    $description = $fetched_data["description"];

    $query_text = "SELECT `y1`, `y2`, `y3`, `y4`, `y5`, `y6`, `y7`, `y8`, `y9`, `y10`, `y11`, `y12` FROM " . $table_name . " WHERE fincode = " . $fincode . " LIMIT 1;";
    $query_one  = mysqli_query($sqlConnect, $query_text);
    $fetched_data = mysqli_fetch_assoc($query_one);
    // $fetched_data = $query_text;

    $fetched_data["description"] = $description;
...
function Wo_GetCompanyGraphPrice($fincode = 0, $timestamp = 0, $type = 'I', $DateOption = '', $DateCount = '', $StartDate = '', $EndDate = ''){
    ...
    $type = Wo_Secure($type);
    $DateOption = Wo_Secure($DateOption);
    $DateCount = Wo_Secure($DateCount);
...
function Wo_GetCompanyPrice($fincode = 0, $timestamp = 1){
    ...
    if (!is_array($fetched_data)){
        return 'Some Issue';
    }else if ($fetched_data['unix_timestamp'] < $timestamp) {
        $fincode = (string)$fincode;
        $timestamp = $timestamp + 210;
        $data = Wo_StockPriceCachingWDownloadNDelete($fincode, $timestamp, $fetched_data['unix_timestamp']);
    }else {
        $fincode = (string)$fincode;
...
function Wo_StockPriceCaching($fincode){
    global $wo, $sqlConnect, $cache;
...
function Wo_GetCompanyPriceFromAPI($fincode, $timestamp, $fetched_timestamp){
    ...
    $exhange = $data['EXCHANGE'];
    $change = $data['CHANGE'];
    $pcnt_change = $data['PER_CHANGE'];
    $open_price = $data['OPEN'];
    $prev_close_price = $data['PREV_CLOSE'];
    $volume = $data['VOLUME'];
    $high = $data['High'];
    $low = $data['LOW'];
    $week_high = $data['52WeekHigh'];
    $week_low = $data['52WeekLow'];
    $mcap = $data['MCAP'];
    $pe = $data['PE'];
    $pe_cons = $data['PE_CONSOL'];
    $book_val = $data['BOOKNAVPERSHARE'];
    $face_val = $data['FV'];
    $price_book = $data['PRICE_BV'];
    $cash_eps = $data['CEPS'];
    $epsc = $data['EPSc'];
    $price_earning = $data['PEPS'];
    $enterprise_val = $data['EV'];
    $ev_ebitda = $data['EV_EBITDA'];
    $ev_sales = $data['EV_SALES'];
    $div_yield = $data['YIELD'];
    $mcap_sales = $data['MCAP_SALES'];
    $price_sales = $data['PRICE_SALES'];

    if ($fetched_timestamp == 0){
        $query_text = "INSERT INTO " . T_STOCKPRICES . " (`fincode`, `last_price`, `change_`, `pcnt_change`, `open_price`, `prev_close_price`, `volume`, `high`, `low`, `week_high`, `week_low`, `mcap`, `pe`, `pe_cons`, `book_val`, `face_val`, `price_book`, `cash_eps`, `epsc`, `price_earning`, `enterprise_val`, `ev_ebitda`, `ev_sales`, `div_yield`, `mcap_sales`, `price_sales`, `exhange`) VALUES ('{$fincode}', '{$last_price}', '{$change}', '{$pcnt_change}', '{$open_price}', '{$prev_close_price}', '{$volume}', '{$high}', '{$low}', '{$week_high}', '{$week_low}', '{$mcap}', '{$pe}', '{$pe_cons}', '{$book_val}', '{$face_val}', '{$price_book}', '{$cash_eps}', '{$epsc}', '{$price_earning}', '{$enterprise_val}', '{$ev_ebitda}', '{$ev_sales}', '{$div_yield}', '{$mcap_sales}', '{$price_sales}', '{$exhange}')";
        $query  = mysqli_query($sqlConnect, $query_text);
    } else{
...
function Wo_StockPriceCachingWDownloadNDelete($fincode, $timestamp, $fetched_timestamp){
    ...
    if ($wo['config']['cacheSystem'] == 1) {
        ...
        if (empty($data)) {
            ...
            $cache->delete($hashed_fincode . '_COMPANY_Price.tmp');
            $data = Wo_GetCompanyPriceFromAPI($fincode, $timestamp, $fetched_timestamp);
            $cache->write($hashed_fincode . '_COMPANY_Price.tmp', $data);
        }
    } else {
...
function SA_deletePortfolio($portfolio_id){
    global $sqlConnect;
    ...
    $query_text = "DELETE FROM " . T_PORTFOLIO_DESC . " WHERE `portfolio_id` = {$portfolio_id}";
    $query  = mysqli_query($sqlConnect, $query_text);
    ...
    $query_one   = "DELETE FROM " . T_PORTFOLIO_BULLION . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    $query_one   = "DELETE FROM " . T_PORTFOLIO_OA . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    $query_one   = "DELETE FROM " . T_PORTFOLIO_OB . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    $query_one   = "DELETE FROM " . T_PORTFOLIO_CASH . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    /* $query_one   = "DELETE FROM " . T_PORTFOLIO_LOANS . " WHERE `portfolio_id` = {$portfolio_id}"; */
    /* $query       = mysqli_query($sqlConnect, $query_one); */

    $query_one   = "DELETE FROM " . T_PORTFOLIO_PROPERTY . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    $query_one   = "DELETE FROM " . T_PORTFOLIO_FD . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    $query_one   = "DELETE FROM " . T_PORTFOLIO_STOCKS . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    $query_one   = "DELETE FROM " . T_PORTFOLIO_MF . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);

    $query_one   = "DELETE FROM " . T_REALIZED . " WHERE `portfolio_id` = {$portfolio_id}";
    $query       = mysqli_query($sqlConnect, $query_one);
...
function AddBullionToPortfolio($bullion_data, $portfolio_id, $auto_add){
    global $sqlConnect;
    ...
    foreach ($bullion_data as $bullion_datum) {
        ...
        $bullion_type = (Wo_Secure($bullion_datum['bullion_type']) == "Gold") ? 1 : 0;
        $bullion_transaction_date = Wo_Secure($bullion_datum['bullion_transaction_date']);
        $bullion_transaction_price = Wo_Secure($bullion_datum['bullion_transaction_price']);
        $bullion_transaction_qty = Wo_Secure($bullion_datum['bullion_transaction_qty']);
        $bullion_charge = Wo_Secure($bullion_datum['bullion_charge']);
        $bullion_net_amount = Wo_Secure($bullion_datum['bullion_net_amount']);
        $bullion_purchased_from = Wo_Secure($bullion_datum['bullion_purchased_from']);
...
function AddFDToPortfolio($fd_data, $portfolio_id, $auto_add){
    global $sqlConnect;
...
function AddStocksToPortfolio($stock_quote_data, $portfolio_id, $no_of_stocks, $auto_add){
    global $wo, $sqlConnect;
    ...
    foreach ($stock_quote_data as $stock_quote_datum) {
        ...
        $query_text   = "SELECT `id` FROM " . T_PORTFOLIO_STOCKS . " WHERE `stock_fincode` = {$stock_fincode} AND `portfolio_id` = {$portfolio_id} AND `SOLD` = 0";
        $query_one    = mysqli_query($sqlConnect, $query_text);
...
function AddDividendToPortfolio($stock_quote_datum, $portfolio_id){
    ...
    AddCashToPortfolio(array(array(
        ...
        'cash_transaction_price' => $amount,
        'note' => ''
    )), $portfolio_id, "Dividend Received From Stocks", false, true);

    $query_text   = "INSERT INTO " . T_PORTFOLIO_STOCKS . " (`portfolio_id`, `stock_fincode`, `stock_transaction_date`, `stock_transaction_price`, `timestamp_created`, `Notes`, `SOLD`) VALUES ({$portfolio_id}, {$stock_fincode}, {$date_}, {$amount}, {$timestamp_created}, '{$dividend_notes}', 3)";
    $query_one    = mysqli_query($sqlConnect, $query_text);
...
function SellStocksFromPortfolio($stock_quote_datum, $portfolio_id, $stocks_available){
    global $wo, $sqlConnect;

    $portfolio_id = Wo_Secure($portfolio_id);
    /* if (!user_portfolio_auth($portfolio_id)) return "The portfolio you are trying to change does not belong to you!"; */
    $stocks_available = Wo_Secure($stocks_available);
    ...
    foreach ($timestamps_sold as $timestamp_sold => $qty){
        ...
        if ($qty == 0){
            $query_text   = "UPDATE " . T_PORTFOLIO_STOCKS . " SET `SOLD` = 1 WHERE `stock_fincode` = {$stock_fincode} AND `portfolio_id` = {$portfolio_id} AND `stock_transaction_date` = {$timestamp_sold} AND `SOLD` = 0;";
            $query_one    = mysqli_query($sqlConnect, $query_text);
        } else if ($qty > 0){
            if ($num_rows <= 0){
                return 'Please do not change system files!';
                ...
                while ($fetched_data = mysqli_fetch_assoc($query_one)) {
                    if (is_array($fetched_data)) {
                        ...
                        if ($qty_left <= 0){
                            ...
                            $query_text   = "INSERT INTO " . T_PORTFOLIO_STOCKS . " (`portfolio_id`, `stock_fincode`, `stock_transaction_date`, `stock_transaction_price`, `stock_transaction_qty`, `timestamp_created`, `Charges`, `Notes`, `SOLD`) VALUES ({$portfolio_id}, {$stock_fincode}, {$fetched_data['stock_transaction_date']}, {$fetched_data['stock_transaction_price']}, {$qty_left}, {$fetched_data['timestamp_created']}, {$fetched_data['Charges']}, '{$fetched_data['Notes']}', 1)";
                            $query_one    = mysqli_query($sqlConnect, $query_text);

                            $query_text   = "UPDATE " . T_PORTFOLIO_STOCKS . " SET `stock_transaction_qty` = `stock_transaction_qty` - {$qty_left} WHERE `stock_fincode` = {$stock_fincode} AND `portfolio_id` = {$portfolio_id} AND `stock_transaction_date` = {$timestamp_sold} AND `timestamp_created` = {$timestamp_created} AND `SOLD` = 0;";
                            $query_one    = mysqli_query($sqlConnect, $query_text);
                            break;
                        }
                    }
    ...
    $query_one   = "UPDATE " . T_PORTFOLIO . " SET `no_of_stocks` = `no_of_stocks` + {$stock_transaction_qty} WHERE `portfolio_id` = {$portfolio_id}";
    $query     = mysqli_query($sqlConnect, $query_one);

    if ($stock_transaction_qty == $stocks_available) {
        $query_one   = "UPDATE " . T_PORTFOLIO . " SET `no_of_unique_stocks` = `no_of_unique_stocks` - 1 WHERE `portfolio_id` = {$portfolio_id}";
        $query     = mysqli_query($sqlConnect, $query_one);
    }

    $query_one   = "INSERT INTO " . T_PORTFOLIO_STOCKS . " (`portfolio_id`, `stock_fincode`, `stock_transaction_date`, `stock_transaction_price`, `stock_transaction_qty`, `timestamp_created`, `Charges`, `Notes`, `SOLD`) VALUES ({$portfolio_id}, {$stock_fincode}, {$stock_transaction_date}, {$stock_transaction_price}, {$stock_transaction_qty}, {$timestamp_created}, {$stock_charge}, '{$stock_note}', 2)";
    $query       = mysqli_query($sqlConnect, $query_one);
    ...
    $wo['portfolio_data']['no_of_stocks'] += $stock_transaction_qty;

    return true;
}
function AddMFToPortfolio($mf_quote_data, $portfolio_id, $auto_add){
    global $sqlConnect;
...
function Wo_ExtraStockDetailInPortfolio($stock_fincode, $portfolio_id) {
    ...
    $query_text = "SELECT `stock_transaction_date`, `stock_transaction_price`, `stock_transaction_qty`, `Charges` FROM " . T_PORTFOLIO_STOCKS . " WHERE `stock_fincode` = {$stock_fincode} AND `portfolio_id` = {$portfolio_id} AND `SOLD` = 0";
    $sql          = mysqli_query($sqlConnect, $query_text);
    ...
    else $realized_gain = $fetched_data['realized_gain'];
    $data['realized_gain'] = $realized_gain;

    $query_text = "SELECT SUM(`stock_transaction_price`) as `total_dividend` FROM " . T_PORTFOLIO_STOCKS . " WHERE `portfolio_id` = {$portfolio_id} AND `stock_fincode` = {$stock_fincode} AND `SOLD` = 3;";
    $sql          = mysqli_query($sqlConnect, $query_text);
    $fetched_data = mysqli_fetch_assoc($sql);
    $data['dividend_amount'] = $fetched_data['total_dividend'];
...
function SA_otherStockTransactions($portfolio_id){
    ...
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
        if (!in_array($fetched_data['stock_fincode'], $stocks)) {
            $stocks[] = $fetched_data['stock_fincode'];
            $query_text = "SELECT `s_name` as `compname` FROM " . T_COMPANIES . " WHERE fincode = {$fetched_data['stock_fincode']};";
            $sql_compname          = mysqli_query($sqlConnect, $query_text);
            $data[mysqli_fetch_assoc($sql_compname)['compname']]   = SA_otherStockTransactionDetails($portfolio_id, $fetched_data['stock_fincode']);
        }
    };

    return $data;
}
function SA_otherStockTransactionDetails($portfolio_id, $stock_fincode) {
    global $sqlConnect;
    ...
    $query_text = "SELECT `stock_transaction_date`, `stock_transaction_price`, `stock_transaction_qty`, `Charges`, `SOLD` FROM " . T_PORTFOLIO_STOCKS . " WHERE `stock_fincode` = {$stock_fincode} AND `portfolio_id` = {$portfolio_id} AND `SOLD` != 0 AND `SOLD` != 3";
    $sql          = mysqli_query($sqlConnect, $query_text);
    while ($fetched_data = mysqli_fetch_assoc($sql)) {
        $data[] = $fetched_data;
    };
...
function SA_getAllTransactions($portfolio_id, $type_) {
    global $sqlConnect;
    ...
    switch ($type_){
        case 'stocks':
            $query_text = "SELECT * FROM " . T_PORTFOLIO_STOCKS . "
                WHERE `portfolio_id` = {$portfolio_id}";
            break;
        default:
            return 'Please Don\'t Change System Files!';
    }
