...
if (isset($_GET['u'])) {
    ...
    if (in_array(true, $check_user)) {
		...
		// ADDED FOR STOCK QUOTES (ADD DESCRIPTION IN COMMENTS
		} else if ($check_user['type'] == 'company') {
            $id                  = $fincode = $check_user['fincode'];
            $wo['company_data']  = Wo_CompanyCaching($fincode);
            $type                = 'stock_quotes';
            ...
            if (isset($_GET['stock'])){
               ...
               if (empty($wo['stock_data'])){
                   ...
                   $about            = $wo['portfolio_data']['portfolio_name'] . ' - ' . 'dasd';
                   $name             = $wo['portfolio_data']['portfolio_name'] . ' - ' . 'asdasd';
                ...
                switch ($_GET['t2']){
                    case 'edit':
                       ...
                       break;
                    case 'edit-stocks':
                       $type             = 'editStocksPortfolio';
                       $about            = $wo['portfolio_data']['portfolio_name'] . ' - edit stocks';
                       $name             = $wo['portfolio_data']['portfolio_name'] . ' - edit stocks';
                       break;
                    case 'add-stocks':
