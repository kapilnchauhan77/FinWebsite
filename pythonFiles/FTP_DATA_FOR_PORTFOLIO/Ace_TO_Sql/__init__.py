        ...
        if int(existing_rows_bse):
            try:
                c.execute('UPDATE bse_indices SET indice = (%s), open = (%s), close = (%s), high = (%s), low = (%s), volume = (%s), date = (%s) WHERE indice = (%s)',
                          [bseDatum['index'], bseDatum['open'], bseDatum['price'], bseDatum['high'], bseDatum['low'], bseDatum['volume'], bseDatum['date'], bseDatum['index']])
            except Exception as e:
                print("Error: {}\n\n".format(str(e)))
            ...
            try:
                c.execute('INSERT INTO bse_indices (indice, open, close, high, low, volume, date) VALUES (%s, %s, %s, %s, %s, %s, %s);',
                          [bseDatum['index'], bseDatum['open'], bseDatum['price'], bseDatum['high'], bseDatum['low'], bseDatum['volume'], bseDatum['date']])
            except Exception as e:
                print("Error: {}\n\n".format(str(e)))
        ...
        if int(existing_rows):
            try:
                c.execute(f'UPDATE stock_prices_{file_type}_ftp_bse SET bse = (%s), open = (%s), close = (%s), high = (%s), low = (%s), volume = (%s), date = (%s) WHERE bse = (%s)',
                          [companyDatumBse['bse'], companyDatumBse['open'], companyDatumBse['price'], companyDatumBse['high'], companyDatumBse['low'], companyDatumBse['volume'], companyDatumBse['date'], companyDatumBse['bse']])
            except Exception as e:
                print("Error: {}\n\n".format(str(e)))
            ...
            try:
                c.execute(f'INSERT INTO stock_prices_{file_type}_ftp_bse (bse, open, close, high, low, volume, date) VALUES (%s, %s, %s, %s, %s, %s, %s);',
                          [companyDatumBse['bse'], companyDatumBse['open'], companyDatumBse['price'], companyDatumBse['high'], companyDatumBse['low'], companyDatumBse['volume'], companyDatumBse['date']])
            except Exception as e:
                print("Error: {}\n\n".format(str(e)))
        ...
        if int(existing_rows):
            try:
                c.execute(f'UPDATE stock_prices_{file_type}_ftp_nse SET nse = (%s), open = (%s), close = (%s), high = (%s), low = (%s), volume = (%s), date = (%s) WHERE nse = (%s)',
                          [companyDatumNse['nse'], companyDatumNse['open'], companyDatumNse['price'], companyDatumNse['high'], companyDatumNse['low'], companyDatumNse['volume'], companyDatumNse['date'], companyDatumNse['nse']])
            except Exception as e:
                print("Error: {}\n\n".format(str(e)))
            ...
            try:
                c.execute(f'INSERT INTO stock_prices_{file_type}_ftp_nse (nse, open, close, high, low, volume, date) VALUES (%s, %s, %s, %s, %s, %s, %s);',
                          [companyDatumNse['nse'], companyDatumNse['open'], companyDatumNse['price'], companyDatumNse['high'], companyDatumNse['low'], companyDatumNse['volume'], companyDatumNse['date']])
            except Exception as e:
                print("Error: {}\n\n".format(str(e)))
